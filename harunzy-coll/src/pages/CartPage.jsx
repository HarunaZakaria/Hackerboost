    ...
    <main className="cart-page">
      ...
      <div className="cart-layout">
        <div className="cart-items" id="cart-items">
          {cartItems.map((item) => (
            <div className="cart-item" key={`${item.id}-${item.selectedSize}`}>
              ...
              <div className="cart-item-total">GHC{item.price * item.qty}</div>
        ...
        <div className="cart-summary" id="cart-summary">
          <h3>Order Summary</h3>
          <div className="summary-row"><span>Subtotal</span><span>GHC{cartTotal}</span></div>
          <div className="summary-row"><span>Shipping</span><span>{cartTotal >= 200 ? "Free" : "$15"}</span></div>
          <div className="summary-divider"></div>
          <div className="summary-row total"><span>Total</span><span>GHC{cartTotal >= 200 ? cartTotal : cartTotal + 15}</span></div>
          {cartTotal < 200 && <p className="free-shipping-note">Add GHC{200 - cartTotal} more for free shipping!</p>}
